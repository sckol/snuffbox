...
static int pvr_socket_write(char *buf, int len);
static int pvr_socket_connect();
...
static int        pvr_socket = INVALID_SOCKET;
...
static int pvr_socket_connect()
{
	...
	if (pvr_socket != INVALID_SOCKET)
	{
		dprintf("%s: Close socket\n", __FUNCTION__);
		close(pvr_socket);
		pvr_socket = INVALID_SOCKET;
	}

	if( stat( STBPVR_SOCKET_FILE, &st) < 0)
	{
		//dprintf("%s: CMD Can't stat " PVR_SOCKET "\n", __FUNCTION__);
		pvr_socket = INVALID_SOCKET;
		return -1;
	}

	if ((pvr_socket = socket(AF_UNIX, SOCK_STREAM, 0)) == -1) {
		eprintf("PVR: Can't create socket\n");
		pvr_socket = INVALID_SOCKET;
		return -1;
	}

	remote.sun_family = AF_UNIX;
	strcpy(remote.sun_path, STBPVR_SOCKET_FILE);
	len = strlen(remote.sun_path) + sizeof(remote.sun_family);
	if (connect(pvr_socket, (struct sockaddr *)&remote, len) == -1) {
		//eprintf("CMD Can't open " pvr_socket "\n");
		close(pvr_socket);
		pvr_socket = INVALID_SOCKET;
		return -1;
	}
...
int pvr_socket_handle_disconnect()
{
	if (pvr_socket != INVALID_SOCKET)
	{
		dprintf("%s: Disconnected from PVR\n", __FUNCTION__);
		close(pvr_socket);
		pvr_socket = INVALID_SOCKET;
	}
	return 0;
...
ssize_t pvr_socket_read(char *buf, int len)
{
	ssize_t res = -1;
	//dprintf("%s: Read %d bytes\n", __FUNCTION__, len);
	if (pvr_socket == INVALID_SOCKET && !pvr_socket_connect())
	{
		//dprintf("%s: Invalid socket\n", __FUNCTION__);
		return -1;
	}
	res = recv(pvr_socket, buf, len, 0);
	//dprintf("%s: Read res %d\n", __FUNCTION__, res);
	if (res <= 0)
	{
		pvr_socket_handle_disconnect();
	}
	return res;
}
...
{
	int res = -1;
	//dprintf("%s: Write '%s' %d bytes\n", __FUNCTION__, buf, len);
	if (pvr_socket == INVALID_SOCKET && !pvr_socket_connect())
	{
		eprintf("%s: Invalid socket\n", __FUNCTION__);
		return -1;
	}
	res = send(pvr_socket, buf, len, 0);
	if (res <= 0)
	{
		eprintf("%s: can't write to socket, res %d: %s\n", __FUNCTION__, res, strerror(errno));
		pvr_socket_handle_disconnect();
	}
	return res;
}
...
static void *pvrThread(void *pArg)
{
	...
	{
		if( (readbytes = pvr_socket_read(buf, sizeof(buf))) > 0)
		{
			do
			{
...
void pvr_startPlaybackDVB(int which)
{
	...
	if( pvr_socket_write( buf, strlen(buf)+1 ) <= 0 )
	{
		interface_showMessageBox( _T("ERR_PVR_RECORD"), thumbnail_error, 5000);
	}
}
