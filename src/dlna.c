...
static void DMP_OnFB_Results (void *fbObj, struct FB_ResultsInfo* results)
{
	...
	dprintf("DLNA: Got results on %d pages err %d cnt %d ptr %08X!\n", FB_GetPageCount(DMP_Browser), resultsInfo->ErrorCode, ILibLinkedList_GetCount(resultsInfo->LinkedList), resultsInfo->LinkedList);
	...
	if (!FB_ContextIsAllRoots(DMP_Browser))
	{
		...
		interface_addMenuEntry((interfaceMenu_t *)&BrowseServersMenu, "..", dlna_fillServerBrowserMenu, parentContext, thumbnail_folder);
		icount++;
	}
	...
	while (node != NULL)
	{
		...
		icount = interface_addMenuEntryCustom((interfaceMenu_t *)&BrowseServersMenu, interfaceMenuEntryText, str, strlen(str)+1, !is_media || cdsObj->Res != NULL, is_media ? dlna_stream_change : dlna_fillServerBrowserMenu, NULL, NULL, NULL, cdsObj, FB_ContextIsAllRoots(DMP_Browser) ? thumbnail_workstation : media_icon);
...
int dlna_start(void) 
{
	...
	dmrObject = DMR_Method_Create(MicroStackChain, 0, "Elecard STB820", "serialNumber", "c588a8d1-f621-44ca-b350-16929549b320", protocolInfo, ILib_Pool);
