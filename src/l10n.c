...
static int l10n_readAndInitTextEntry(char* entrySource, int entryIndex)
{
	...
	if (delimeter == NULL)
		return -1;
	l10n_textEntries[entryIndex].key = (char*)dmalloc( sizeof(char) * (delimeter-entrySource)+1 );
	strncpy(l10n_textEntries[entryIndex].key, entrySource, delimeter-entrySource);
	l10n_textEntries[entryIndex].key[(delimeter-entrySource)] = 0;
