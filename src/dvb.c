...
static struct dvb_instance dvbInstances[4];
static long currentFrequency[4];
...
static int dvb_sectionParse (long frequency, char* demux_devname, struct section_buf *s)
{
	...
	dprintf("DVB: parsing, media type %d, freq %lu\n", media.type, media.frequency);
	...
	if (!get_bit(s->section_done, section_number))
	{
		...
		case 0x00:
			...
			//dvb_parse_pat (frequency, demux_devname, buf, section_length, table_id_ext);
			cur = *services;
			while ( cur != NULL)
			{
				service = (EIT_service_t*)cur->data;
				if( (service->flags & serviceFlagHasPAT) && service->media.frequency == (unsigned long)frequency )
				{
					pmt_filter = dmalloc(sizeof(struct section_buf));
					if( pmt_filter != NULL )
					{
						dprintf("DVB: new pmt filter ts %d, pid %d, service %d, media %d\n", service->common.transport_stream_id, service->program_map.program_map_PID, service->common.service_id, service->common.media_id);
						dvb_filterSetup(pmt_filter, demux_devname, service->program_map.program_map_PID, 0x02, 2, s->nit, services);
		...
		case 0x02:
			...
			{
				service = ((EIT_service_t*)cur->data);
				//dprintf("DVB: Check pmt for: pid %d/%d, media %lu/%lu, tsid %d/%d, freqs %lu/%lu, sid %d\n", service->program_map.program_map_PID, s->pid, service->common.media_id, MEDIA_ID, service->common.transport_stream_id, s->transport_stream_id, service->media.frequency, (unsigned long)frequency, service->common.service_id);
				if (service->program_map.program_map_PID == s->pid && service->common.media_id == MEDIA_ID && service->common.transport_stream_id == s->transport_stream_id && service->media.frequency == (unsigned long)frequency )
				{
					dprintf("DVB: parse pmt filter ts %d, pid %d, service %d, media %d\n", service->common.transport_stream_id, service->program_map.program_map_PID, service->common.service_id, service->common.media_id);
					updated = parse_pmt(services, (unsigned char *)s->buf, service->common.transport_stream_id, MEDIA_ID, &media);
				}
				cur = cur->next;
			}
			mysem_release(dvb_semaphore);
			//dvb_parse_pmt (buf, section_length, table_id_ext, frequency);
			break;
    ...
    case 0x40:
			...
			if (s->nit != NULL)
			{
				...
				mysem_release(dvb_semaphore);
				//dvb_parse_nit(buf, section_length, table_id_ext, frequency);
			}
			break;
    ...
    case 0x46:
			...
			mysem_release(dvb_semaphore);
			//dvb_parse_sdt (frequency, buf, section_length, table_id_ext);
			break;
		case 0x4E:
		case 0x50:
		case 0x51:
...
static int dvb_setFrequency(fe_type_t  type, long frequency, int frontend_fd, int tuner, int wait_for_lock, EIT_media_config_t *media, dvb_cancelFunctionDef* pFunction)
{
...
/*
	eprintf("DVB: Current frequency %d, new frequency %d\n", currentFrequency[tuner], frequency);
	...
	if (currentFrequency[tuner] != frequency)*/
	{
		...
		if (type == FE_OFDM && (media == NULL || media->type == serviceMediaDVBT))
		{
			...
			p.inversion = appControlInfo.dvbtInfo.fe.inversion == 2 ? INVERSION_AUTO : (appControlInfo.dvbtInfo.fe.inversion == 1 ? INVERSION_ON : INVERSION_OFF);
		}
		else if (type == FE_QAM && (media == NULL || media->type == serviceMediaDVBC))
		{
			if (media != NULL)
			{
				...
				eprintf("DVB: Symbol rate %ld, modulation %ld\n", p.u.qam.symbol_rate, p.u.qam.modulation);
			} else
			{
				p.u.qam.modulation  = appControlInfo.dvbcInfo.modulation;
				p.u.qam.symbol_rate = appControlInfo.dvbcInfo.symbolRate*1000;
			}
			p.inversion = appControlInfo.dvbcInfo.fe.inversion == 2 ? INVERSION_AUTO : (appControlInfo.dvbcInfo.fe.inversion == 1 ? INVERSION_ON : INVERSION_OFF);
...
static int dvb_checkFrequency(fe_type_t  type, __u32 * frequency, int frontend_fd, int tuner, __u32* ber, long fe_step_size, EIT_media_config_t *media, dvb_cancelFunctionDef* pFunction)
{
	int res;
	...
	if ((res = dvb_setFrequency(type, *frequency, frontend_fd, tuner, 1, media, pFunction)) == 0)
	{
		...
		for (i = 0; i < 10; i++)
		{
			...
			while (1)
			{
				// Get the front-end state including current freq. setting. Some drivers may need to be modified to update this field.
	...
	switch (dvb_getType(tuner)) {
		...
		case FE_QAM:
			...
			*/
			break;
		case FE_QPSK:
			eprintf("DVB: Satellite FEs not yet supported");
			return -1;
			break;
		case FE_ATSC:
...
int dvb_getSignalInfo(tunerFormat tuner, uint16_t *snr, uint16_t *signal, uint32_t *ber, uint32_t *uncorrected_blocks)
{
	...
	sprintf(frontend_devname, "/dev/dvb/adapter%d/frontend0", tuner);

	if ((frontend_fd = open(frontend_devname, O_RDONLY | O_NONBLOCK)) < 0)
	{
		eprintf("DVB: failed opening '%s'\n", frontend_devname);
#ifndef ENABLE_PVR
...
int dvb_serviceScan( tunerFormat tuner, dvb_displayFunctionDef* pFunction)
{
	...
	if(!appControlInfo.dvbCommonInfo.streamerInput)
	{
		...
		{
			eprintf("DVB: Scanning DVB FrontEnd-%d  Model=%s,  Type=%s ; ", tuner, fe_info.name, gFE_Type_Names[fe_info.type]);
		...
		{
			eprintf("DVB: Scanning DVB FrontEnd-%d  Model=%s,  Type=%s\n, NOT SUPPORTED", tuner, fe_info.name, gFE_Type_Names[fe_info.type]);
		...
		for(frequency = low_freq; frequency <= high_freq; frequency += freq_step)
		{
			...
			if (1== dvb_checkFrequency(fe_info.type, &f, frontend_fd, tuner, &match_ber, fe_info.frequency_stepsize, NULL, NULL))
			{
					found = 1;
			}
...
int dvb_frequencyScan( tunerFormat tuner, __u32 frequency, EIT_media_config_t *media, dvb_displayFunctionDef* pFunction, NIT_table_t *scan_network, int save_service_list, dvb_cancelFunctionDef* pCancelFunction)
{
	...
	if(!appControlInfo.dvbCommonInfo.streamerInput)
	{
		...
		if (1== (err = dvb_checkFrequency(fe_info.type, &f, frontend_fd, tuner, &match_ber, fe_info.frequency_stepsize, media, pCancelFunction)))
		{
			found = 1;
		}
		if (err == -1)
		...
		if (save_service_list == -1)
		{
			for(;;)
			{
				if (pFunction == NULL || pFunction(frequency, 0, tuner, current_frequency_number, max_frequency_number) == -1)
				{
	...
	if (scan_network != NULL)
	{
		...
		while (tstream_element != NULL)
		{
			...
			if (tsrtream != NULL && tsrtream->media.type > serviceMediaNone && tsrtream->media.frequency > 0 && tsrtream->media.frequency != frequency)
			{
		...
		while (tstream_element != NULL)
		{
			...
			if (tsrtream != NULL && tsrtream->media.type > serviceMediaNone && tsrtream->media.frequency > 0 && tsrtream->media.frequency != frequency)
			{
				if (pFunction != NULL && pFunction(tsrtream->media.frequency, dvb_getNumberOfServices(), tuner, current_frequency_number, max_frequency_number) == -1)
				{
					dprintf("DVB: BREAK: pFunction(f, serviceCount, tuner) == -1\n");
					break;
				}
				eprintf("DVB: Network Scan: %lu Hz\n", tsrtream->media.frequency);
				dvb_frequencyScan(tuner, tsrtream->media.frequency, &tsrtream->media, pFunction, NULL, 0, pCancelFunction);
...
static int dvb_serviceChange(struct dvb_instance * dvb, EIT_media_config_t *media)
{
	...
	dprintf("%s: setting filterv=%d filtera=%d filtert=%d filterp=%d\n", __FUNCTION__, dvb->filterv.pid, dvb->filtera.pid, dvb->filtert.pid, dvb->filterp.pid );
#ifdef ENABLE_PVR
...
#endif
	{
		ret = dvb_setFrequency(dvb->fe_type, dvb->setFrequency, dvb->fdf, dvb->vmsp, 0, media, NULL);
		if (ret < 0)
		{
	...
	if( dvb->mode == DvbMode_Multi )
	{
		...
		dprintf("%s: filterv1=%d filterv2=%d filterv3=%d\n", __FUNCTION__, dvb->filterv1.pid,dvb->filterv2.pid,dvb->filterv3.pid);
	} else
#endif
	{
...
#ifdef STBTI
		if (dvb->filterp.pid != dvb->filtera.pid && dvb->filterp.pid != dvb->filterv.pid && dvb->filterp.pid != dvb->filtert.pid)
		{
...
static int open_dvb_instance (struct dvb_instance * dvb, int vmsp, char *pFilename)
{
	...
	if (!appControlInfo.dvbCommonInfo.streamerInput && (dvb->mode != DvbMode_Play))
	{
		...
		sprintf(path, "/dev/dvb/adapter%i/frontend0", vmsp);
...
#endif
		{
			...
			{
				eprintf("DVB: FrontEnd-%d  Model=%s,  Type=%s\n, NOT SUPPORTED by this app.", vmsp, fe_info.name, gFE_Type_Names[fe_info.type]);
				return -1;
			}
			else
			{
...
static void *dvb_teletextThread(void *pArg)
{
	...
	do
	{
		...
		if (poll(pfd,1,1))
		{
			...
			if (pfd[0].revents & POLLIN)
			{
				length = read(dvb->fdin, buf, TELETEXT_PACKET_BUFFER_SIZE);
				if (length < 0)
				{
...
int dvb_getNumberOfServices(void)
{
	...
	int serviceCount;
	mysem_get(dvb_semaphore);
	for( service_element = dvb_services, serviceCount = 0; service_element != NULL; service_element = service_element->next )
	{
...
int dvb_hasMediaNB(EIT_service_t *service)
{
	if ( (service->flags & (serviceFlagHasPAT|serviceFlagHasPMT)) == (serviceFlagHasPAT|serviceFlagHasPMT) &&
		(dvb_hasMediaTypeNB(service, mediaTypeVideo) || dvb_hasMediaTypeNB(service, mediaTypeAudio) || dvb_hasMediaTypeNB(service, payloadTypeText)) )
	{
		return 1;
	}
	return 0;
...
int dvb_hasMediaTypeNB(EIT_service_t *service, media_type m_type)
{
	...
	while( stream_element != NULL)
	{
		...
		if( (m_type == mediaTypeVideo && (type == payloadTypeH264 || type == payloadTypeMpeg2 || type == payloadTypeMpeg4)) ||
			(m_type == mediaTypeAudio && (type == payloadTypeMpegAudio || type == payloadTypeAAC || type == payloadTypeAC3)) ||
...
int dvb_getServiceURL(EIT_service_t *service, char* URL)
{
	...
	sprintf(URL,"dvb://%lu:%hu/?pcr=%hu",(unsigned long)frequency,service->common.service_id,service->program_map.map.PCR_PID);
	URL+=strlen(URL);
	stream_element = service->program_map.map.streams;
	while( stream_element != NULL)
	{
...
char* dvb_getEventName( EIT_service_t* service, short int event_id )
{
	...
	{
		for( event_element = service->schedule; event_element != NULL && (event = (EIT_event_t*)event_element->data) != NULL; event_element = event_element->next )
		{
...
int dvb_startDVB(DvbParam_t *pParam)
{
	...
	if ((pParam->vmsp < 0) || (pParam->vmsp > 3))
	{
		eprintf("DVB: Invalid vmsp - use range 0-3\n");
		return -1;
	}
	...
	if (pParam->mode == DvbMode_Play || pParam->frequency > 0)
	{
		...
		if ( dvb->mode == DvbMode_Multi )
		{
			eprintf("%s: Multi %d %d %d %d\n", __FUNCTION__,pParam->param.multiParam.channels[0],pParam->param.multiParam.channels[1],pParam->param.multiParam.channels[2],pParam->param.multiParam.channels[3]);
...
#endif
		{
			eprintf("%s: Play Video pid %d and Audio pid %d Text pid %d Pcr %d\n", __FUNCTION__, pParam->param.playParam.videoPid, pParam->param.playParam.audioPid, pParam->param.playParam.textPid, pParam->param.playParam.pcrPid);
...
#ifndef ENABLE_PVR
		if (dvb_getServiceFrequency( dvb_getService( pParam->param.liveParam.channelIndex ), &dvb->setFrequency) != 0)
#else
		if( (s32)pParam->frequency < 0 )
		{
			dvb->setFrequency = -1;
		} else if (dvb_getServiceFrequency( dvb_getService( pParam->param.liveParam.channelIndex ), &dvb->setFrequency) != 0)
#endif
		{
...
void dvb_init(void)
{
	...
	{
		char tunerString[256];

		/* Create the name of the tuner device */
		sprintf(tunerString, "/dev/dvb/adapter%d/frontend0", i);

		appControlInfo.tunerInfo[i].status = helperFileExists(tunerString) ? tunerInactive : tunerNotPresent;

		dprintf("%s: tuner %d: '%s' %d\n", __FUNCTION__, i, tunerString, appControlInfo.tunerInfo[i].status);
	}
	...
	for(i=0; i<4; i++)
	{
		...
		sprintf(path, "/dev/dvb/adapter%i/frontend0", i);
		...
		close(fdf);
		gFE_type = fe_info.type;
		break;  // Assume all FEs are the same type !
	}
