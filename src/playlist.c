...
int playlist_streamStart()
{
	...
	return playlist_stream_change((interfaceMenu_t *)&playlistMenu, (void*)streamIndex);
}
...
int playlist_startNextChannel(int direction, void* pArg)
{
	...
	return playlist_stream_change((interfaceMenu_t *)&playlistMenu, (void*)new_index);
}
...
static int playlist_stream_change(interfaceMenu_t *pMenu, void* pArg)
{
	...
	URL = playlist_getURL((int)pArg);
	dprintf("%s: %d = %s\n", __FUNCTION__,(int)pArg,URL);
	if (URL != NULL)
	{
		appControlInfo.playbackInfo.channel = (int)pArg + 1;
...
int playlist_setChannel(int channel, void* pArg)
{
	...
	channel--;
	return playlist_stream_change( (interfaceMenu_t*)&playlistMenu, (void*)channel );
}
...
static int playlist_saveURL(interfaceMenu_t *pMenu, char *value, void* pArg)
{
	...
	if((res = m3u_getEntry( PLAYLIST_FILENAME, (int)pArg)) != 0)
	{
		interface_showMessageBox(_T("ERR_SAVE_FAILED"), thumbnail_error, 0);
		return res;
	}
	strcpy(description, m3u_description);
	if((res = m3u_replaceEntryByIndex( PLAYLIST_FILENAME, (int)pArg, value, description )) != 0)
	{
		interface_showMessageBox(_T("ERR_SAVE_FAILED"), thumbnail_error, 0);
	} else
	{
...
static int playlist_saveDescription(interfaceMenu_t *pMenu, char *value, void* pArg)
{
	...
	strcpy(url, m3u_url);
	if((res = m3u_replaceEntryByIndex( PLAYLIST_FILENAME, (int)pArg, url, value )) != 0)
	{
		interface_showMessageBox(_T("ERR_SAVE_FAILED"), thumbnail_error, 0);
	} else
	{
...
{
	int res;
	if( (res = m3u_deleteEntryByIndex( PLAYLIST_FILENAME, (int)pArg)) != 0)
	{
		interface_showMessageBox(_T("ERR_SAVE_FAILED"), thumbnail_error, 0);
	}
	return res;
...
{
	return num == 0 ? playlist_getDescription((int)pArg) : NULL;
}
...
static int playlist_fillMenu(char *value)
{
	...
	if(file != NULL)
	{
		while ( m3u_readEntry(file) == 0 )
		{
			...
			interface_addMenuEntry((interfaceMenu_t*)&playlistMenu, desc, playlist_stream_change, (void*)i, icon );
			i++;
