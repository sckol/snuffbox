...
int voip_pipe_connect()
{
	...
	//dprintf("%s: Connect socket\n", __FUNCTION__);

	if (voip_socket != INVALID_SOCKET)
	{
		eprintf("VoIP: Close socket\n");
		close(voip_socket);
		voip_socket = INVALID_SOCKET;
	}

	if( stat( VOIP_SOCKET, &st) < 0)
	{
		//dprintf("%s: Can't stat " VOIP_SOCKET "\n", __FUNCTION__);
		voip_socket = INVALID_SOCKET;
		return -1;
	}

	if ((voip_socket = socket(AF_UNIX, SOCK_STREAM, 0)) == -1) {
		eprintf("VoIP: Can't create socket\n");
	voip_socket = INVALID_SOCKET;
	return -1;
	}

	remote.sun_family = AF_UNIX;
	strcpy(remote.sun_path, VOIP_SOCKET);
	len = strlen(remote.sun_path) + sizeof(remote.sun_family);
	if (connect(voip_socket, (struct sockaddr *)&remote, len) == -1) {
	//eprintf("%s: CMD Can't open " VOIP_SOCKET "\n", __FUNCTION__);
	close(voip_socket);
	voip_socket = INVALID_SOCKET;
	return -1;
	}
...
int voip_pipe_handle_disconnect()
{
	if (voip_socket != INVALID_SOCKET)
	{
		eprintf("VoIP: Disconnected from pjsua\n");
		close(voip_socket);
		voip_socket = INVALID_SOCKET;
...
int voip_pipe_read(char *buf, int len)
{
	int res = -1;

	//dprintf("%s: Read %d bytes\n", __FUNCTION__, len);

	if (voip_socket == INVALID_SOCKET && !voip_pipe_connect())
	{
		//eprintf("VoIP: Invalid socket\n");
		return -1;
	}

	res = recv(voip_socket, buf, len, 0);

	//dprintf("%s: Read res %d\n", __FUNCTION__, res);

	if (res <= 0)
	{
		voip_pipe_handle_disconnect();
	}

	return res;
}

int voip_pipe_write(char *buf, int len)
{
	int res = -1;
	...
	res = send(voip_socket, buf, len, 0);

	//dprintf("%s: Write res %d\n", __FUNCTION__, res);
