...
//extern void *rtp_main(void *pArg);
...
#ifndef USE_PIPE
	#define RTP_DEVICE        "/dev/dvb/adapter2/demux0"
#else
	#ifdef STBxx
		#define PVR_PIPE_NAME   "/tmp/dsppipe_ts.ts_v_mpeg2"
	#else
		#include <sys/socket.h>
		#include <sys/un.h>
		#define PVR_PIPE_NAME   "/tmp/dsppipe_ts.mpg"
	#endif
	#define RTP_DEVICE        PVR_PIPE_NAME
#endif
...
/*
#ifdef USE_PCR
#define PCR_FD_PTR &rtp[which].fdp
#else
#define PCR_FD_PTR NULL
#endif
*/
...
//	createListMenu(&rtpMenu, _T("TV_CHANNELS"), thumbnail_multicast, rtp_icons, pParent,
...
//				   NULL, NULL, NULL);
	...
	for ( which=0; which<screenOutputs; which++ )
	{
		createListMenu(&rtpStreamMenu[which], _T("TV_CHANNELS_LIST"), thumbnail_multicast, rtp_icons, pParent,
					   /* interfaceInfo.clientX, interfaceInfo.clientY,
					   interfaceInfo.clientWidth, interfaceInfo.clientHeight,*/ interfaceListMenuIconThumbnail,
					   NULL, rtp_stopCollect, (void *)which);
...
static void rtp_setStateCheckTimer(int which, int bEnable)
{
	...
	if (bEnable)
	{
		interface_addEvent(rtp_stateTimerEvent, (void*)STREAM_INFO_SET(which, 0), 1000, 1);
	} else
	{
		interface_removeEvent(rtp_stateTimerEvent, (void*)STREAM_INFO_SET(which, 0));
	}
}
...
void rtp_stopVideo(int which)
{
	...
	if ( appControlInfo.rtpInfo[which].active != 0 )
	{
		...
		rtp_checkStream((void*)STREAM_INFO_SET(which, 0));
...
//		}
		...
		rtp[which].selectedDesc.media[0].fmt == payloadTypeMpeg2)
	{
		...
		if( appControlInfo.rtpMenuInfo.channel >= 0 && rtp_info[appControlInfo.rtpMenuInfo.channel].video_type > 0 )
		{
	...
	if ( streams.count > 0 )
	{
		...
		for ( ; streamNumber<streams.count; streamNumber++ )
		{
			...
			if( genre_index != GENRE_COUNT && genre != cur_genre )
			{
				...
				if( str != NULL )
				{
					interface_addMenuEntry((interfaceMenu_t*)&rtpStreamMenu[which], str, (menuActionFunction)menuDefaultActionShowMenu, (void*)&rtpGenreMenu, -1);
				}
			}
			...
			switch( streams.items[sortedNumber].media[0].proto )
			{
				...
				case mediaProtoUDP:
					if ( streams.items[sortedNumber].media[0].fmt != payloadTypeUnknown && rtp_engine_supports_transport(RTP_ENGINE, streams.items[sortedNumber].media[0].proto ))
					{
						...
						interface_addMenuEntryCustom((interfaceMenu_t*)&rtpStreamMenu[which], interfaceMenuEntryText,  channelEntry, strlen(channelEntry)+1, 1, rtp_stream_change,
						  ...
						  NULL, NULL,
#endif
						  rtp_menuEntryDisplay, (void*)STREAM_INFO_SET(which, sortedNumber), thumbnail_multicast);
						if( last_channel_is_udp &&
				...
				case mediaProtoRTMP:
					...
					interface_addMenuEntryCustom((interfaceMenu_t*)&rtpStreamMenu[which], interfaceMenuEntryText,  channelEntry, strlen(channelEntry)+1, 1, rtp_stream_change,
					  ...
					  NULL, NULL,
#endif
					  rtp_menuEntryDisplay, (void*)STREAM_INFO_SET(which, sortedNumber), (streams.items[sortedNumber].media[0].proto == mediaProtoRTSP ? thumbnail_vod : thumbnail_internet));
...
/*					    (playlistModeIPTV == appControlInfo.playbackInfo.playlistMode && 0 == strcmp( appControlInfo.rtpMenuInfo.lastUrl, rtp_info[streamNumber].url )) ||
	...
	if (new_index >= 0)
		rtp_stream_change(interfaceInfo.currentMenu, (void*)STREAM_INFO_SET(which, new_index));
...
static int rtp_stream_change(interfaceMenu_t *pMenu, void* pArg)
{
	...
	if ( rtp_stream_start(pArg) == 0 )
	{
		...
		//interface_menuActionShowMenu(pMenu, (void*)&rtpMenu);
...
int rtp_setChannel(int channel, void* pArg)
{
	...
	if( streams.items[channel].media[0].proto == mediaProtoRTP || streams.items[channel].media[0].proto == mediaProtoUDP )
		rtp_stream_selected((interfaceMenu_t*)&rtpStreamMenu[which], (void*)STREAM_INFO_SET(which, channel));
#endif
	rtp_stream_change((interfaceMenu_t*)&rtpStreamMenu[which], (void*)STREAM_INFO_SET(which, channel));
...
void rtp_getPlaylist(int which)
{
	...
	} else if ( rtp_sap_collected == 0 )
	{
		...
		ret = pthread_create(&rtp[which].collectThread, NULL, stream_list_updater, (void*)which);
		ret = pthread_detach(rtp[which].collectThread);
	}
}
...
static int rtp_setChannelFromURL(interfaceMenu_t *pMenu, char *value, char *description, char *thumbnail, void* pArg)
{
	...
	return rtp_stream_change(pMenu, (void*)STREAM_INFO_SET(which, channelNumber));
}
...
{
	return rtp_setChannelFromURL(interfaceInfo.currentMenu, value, description, thumbnail, (void*)which);
}
...
{
	genre_index = (unsigned int)pArg & 0x00FF;
	rtp_fillStreamMenu(screenMain);
	interface_menuActionShowMenu(pMenu, (void*)&rtpStreamMenu[screenMain]);
	return 0;
}
...
static int rtp_fillGenreMenu()
{
	...
	str = _T("ALL");
	genre = GENRE_ALL;
	interface_addMenuEntry((interfaceMenu_t*)&rtpGenreMenu, str, rtp_changeGenre, (void*)genre, -1);
	if( genre == genre_index )
	{
...
static int rtp_keyCallback(interfaceMenu_t *pMenu, pinterfaceCommandEvent_t cmd, void* pArg)
{
	...
	dprintf("%s: cmd %d selected %d pArg 0x%08x (%d %d)\n", __FUNCTION__, cmd->command, streamNumber, pArg, which, STREAM_INFO_GET_STREAM((int)pArg));
	...
	if( cmd->command == interfaceCommandRefresh )
	{
		...
		rtp_initStreamMenu( pMenu, (void*)screenMain );
		return 0;
	}
	...
	if( streamNumber < 0 )
	{
		switch(cmd->command)
		{
			...
			case interfaceCommandBlue:
				pvr_initPvrMenu( pMenu, (void*)pvrJobTypeRTP );
				return 0;
			default:;
		}
	}
		...
		case interfaceCommandGreen:
			...
			if( appControlInfo.rtpMenuInfo.usePlaylistURL && rtp_info[streamNumber].poster && rtp_info[streamNumber].poster[0] )
			{
				...
				interface_showPosterBox( ptr,
											...
											thumbnail_multicast,
											rtp_info[streamNumber].poster, NULL, (void*)streamNumber);
			} else
		...
		case interfaceCommandTV:
			if((strncmp( rtp_info[streamNumber].url, URL_UDP_MEDIA, sizeof(URL_UDP_MEDIA)-1 ) == 0 ||
				strncmp( rtp_info[streamNumber].url, URL_RTP_MEDIA, sizeof(URL_RTP_MEDIA)-1 ) == 0 ) )
				{
					...
					rtp_multiviewPlay(pMenu, (void*)CHANNEL_INFO_SET(which, CHANNEL_CUSTOM));
				return 0;
...
int rtp_showEPG(int which, playControlSetupFunction pSetup)
{
	...
	if( rtp_getProgramInfo( appControlInfo.rtpMenuInfo.channel, program_offset, rtpInfoTypeShort ) == 0 && channel_buff[0] != 0 )
	{
		...
		interfacePlayControl.pArg = (void*)STREAM_INFO_SET(which, appControlInfo.rtpMenuInfo.channel);
...
static int rtp_shortInfoProcessCommand(pinterfaceCommandEvent_t cmd, void* pArg)
{
	int channelNumber = STREAM_INFO_GET_STREAM((int)pArg);
	int which = STREAM_INFO_GET_SCREEN((int)pArg);
		...
		case interfaceCommandPageUp:
			if( channelNumber > 0 )
			{
				if( rtp_getProgramInfo( channelNumber-1, 0, rtpInfoTypeShort ) == 0 && channel_buff[0] != 0 )
				{
					program_offset = 0;
					channelNumber--;
					interfacePlayControl.pArg = (void*)STREAM_INFO_SET(which, channelNumber);
					interface_playControlRefresh(1);
				} else
				{
					interface_showMessageBox(_T("ERR_CONNECT"), thumbnail_epg, RTP_EPG_ERROR_TIMEOUT);
		...
		case interfaceCommandPageUp:
			if( channelNumber > 0 )
			{
				if( rtp_getProgramInfo( channelNumber-1, 0, rtpInfoTypeFull ) == 0 && channel_buff[0] != 0 )
				{
					...
					channelNumber--;
					interfacePlayControl.pArg = (void*)STREAM_INFO_SET(which, channelNumber);
					interface_showScrollingBox( rtpEpgInfo.program.info, thumbnail_multicast, rtp_longInfoCallback, (void*)STREAM_INFO_SET(which, channelNumber));
				} else
				{
					interface_showMessageBox(_T("ERR_CONNECT"), thumbnail_epg, RTP_EPG_ERROR_TIMEOUT);
				}
...
int rtp_initEpgMenu(interfaceMenu_t *pMenu, void* pArg)
{
	...
	for( element = rtp_info[channelNumber].schedule; element != NULL; element=element->next )
	{
		...
		interface_addMenuEntryCustom( (interfaceMenu_t *)&rtpEpgMenu, interfaceMenuEntryText, text, strlen(text)+1, 1, NULL, NULL, NULL,
			...
			NULL,
#endif
		(void*)CHANNEL_INFO_SET(event->event_id,channelNumber), -1 );
	}
	mysem_release(rtp_epg_semaphore);

	return interface_menuActionShowMenu(pMenu, (void*)&rtpEpgMenu );
}
...
static int rtp_epgKeyCallback(interfaceMenu_t *pMenu, pinterfaceCommandEvent_t cmd, void* pArg)
{
	...
	switch(cmd->command)
	{
		case interfaceCommandRecord:
			...
			channelNumber = CHANNEL_INFO_GET_CHANNEL( (int)rtpEpgMenu.baseMenu.menuEntry[rtpEpgMenu.baseMenu.selectedItem].pArg );
			event_id      = CHANNEL_INFO_GET_SCREEN(  (int)rtpEpgMenu.baseMenu.menuEntry[rtpEpgMenu.baseMenu.selectedItem].pArg );
...
static int rtp_playControlProcessCommand(pinterfaceCommandEvent_t cmd, void *pArg)
{
	...
	if( cmd->command == interfaceCommandTV)
	{
		if(appControlInfo.rtpMenuInfo.channel != CHANNEL_CUSTOM)
		{
			...
			{
				rtp_multiviewPlay( (interfaceMenu_t*)&rtpStreamMenu[screenMain], (void*)CHANNEL_INFO_SET(screenMain, appControlInfo.rtpMenuInfo.channel) );
			} else
				rtp_multiviewPlay( (interfaceMenu_t*)&rtpStreamMenu[screenMain], (void*)CHANNEL_INFO_SET(screenMain, CHANNEL_CUSTOM) );
			return 0;
		}
	}
	return 1;
...
static int rtp_multiviewNext(int direction, void* pArg)
{
	...
	return rtp_multiviewPlay( interfaceInfo.currentMenu, (void*)CHANNEL_INFO_SET(which, newIndex) );
}
...
static int rtp_multiviewPlay(interfaceMenu_t *pMenu, void *pArg)
{
	...
	for( i = streamNumber+1; mvCount<4 && i<streams.count; i++ )
	{
		...
		{
			appControlInfo.multiviewInfo.pArg[mvCount++] = (void*)CHANNEL_INFO_SET(which, i);
		}
	}
