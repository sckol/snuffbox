...
int loadAppSettings()
{
	...
	while (fgets(buf, BUFFER_SIZE, fd) != NULL)
	{
		...
		else if (sscanf(buf, "QAMMODULATION=%ld", &appControlInfo.dvbcInfo.modulation) == 1)
		{
			//dprintf("%s: dvb modulation %d\n", __FUNCTION__, appControlInfo.dvbModulation);
		}
		else if (sscanf(buf, "QAMSYMBOLRATE=%ld", &appControlInfo.dvbcInfo.symbolRate) == 1)
		{
			//dprintf("%s: dvb sym rate %d\n", __FUNCTION__, appControlInfo.dvbSymbolRate);
		}
...
set_overrides:
	...
	getParam(BROWSER_CONFIG_FILE, "HTTPProxyServer", "", buf);
	if( buf[0] != 0 )
		setenv("http_proxy", buf, 1);
	else
		unsetenv("http_proxy");
	getParam(BROWSER_CONFIG_FILE, "HTTPProxyLogin", "", buf);
	if( buf[0] != 0 )
	{
		setenv("http_proxy_login", buf, 1);
		getParam(BROWSER_CONFIG_FILE, "HTTPProxyPasswd", "", buf);
		setenv("http_proxy_passwd", buf, 1);
	}
	else
	{
...
int saveAppSettings()
{
	...
	int dontUpdateSource = appControlInfo.mediaInfo.bHttp && appControlInfo.playbackInfo.playlistMode != playlistModeIPTV && appControlInfo.playbackInfo.streamSource != streamSourceYoutube;
	...
	fprintf(fd, "MEDIA_FILTER=%s\n",              appControlInfo.mediaInfo.typeIndex < 0 ? "showall" : mediaTypeNames[appControlInfo.mediaInfo.typeIndex] );
...
#ifdef ENABLE_DVB_DIAG
	fprintf(fd, "PROFILE_DIAGNOSTICS=%d\n",       appControlInfo.offairInfo.diagnosticsMode == DIAG_FORCED_OFF ? DIAG_FORCED_OFF : DIAG_ON);
#endif
#ifdef ENABLE_VOIP
...
#endif
	appControlInfo.timeInfo.displayed = 0;
